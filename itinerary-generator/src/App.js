...
const genAI = new GoogleGenerativeAI(''); // add your API key here for Gemini Pro to run the generator -> https://makersuite.google.com/app/apikey
...
const topLocations = [
  ...
  { name: "Goa", value: "Goa" },
  // we can add more locations or pick through ML model
];

const defaultValues = {
  destinationCountry: "",
  departure: "",
  budgetType: options.budgetTypes[2],
  travelStyle: options.travelStyles[2],
  interestsNew: [],
...
const Main = ({ loading, response, handleSubmit, handleChange, email }) => (
  <MainContent>
    ...
    {!response && <Subtitle>Fill the form to generate itinerary</Subtitle>}
    ...
    <>
      <Container>
        ...
        <Panel>
          ...
          <Label htmlFor="destinationCountry">Departure</Label>
            <Input
              type="text"
              id="departure"
              name="departure"
            ...
            <FormGroup>
                ...
                <Select
                  ...
                  onChange={handleChange}
                >
                  {options.budgetTypes.map((option) => (
